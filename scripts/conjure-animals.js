...
try {
	...
	if (args[0].macroPass === "preActiveEffects") {
		...
		if (choice) {
			...
			const collectionName = resultData.collection;
			const summonId = resultData.resultId;
			...
			const name = "Summoned (" + entity.data.name + ")";
			let updates = {
				token: {
					"name": name,
					"disposition": 1,
			...
			if (mightySummoner) {
				...
				let originalHPFormula = entity.data.data.attributes.hp.formula;
			...
			let weapons = document.items.filter(i => i.data.type === `weapon`);
			if (weapons) {
			...
			// Spawn the result
			const tokenData = entity.data.token;
			const maxRange = 60;
			let snap = tokenData.width/2 === 0 ? 1 : -1;
			let {x, y} = await MidiMacros.warpgateCrosshairs(token, maxRange, optionName, tokenData.img, tokenData, snap);
		...
		game.actors.forEach(t => { if (!t.data.folder && (t.data.name === summonName)) { t.delete(); } });
