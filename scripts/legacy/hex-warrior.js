/*
	...
	When you attack with that weapon, you can use your Charisma modifier, instead of Strength or Dexterity, for the
...
try {
	if (args[0] === "on") {
		...
		let weapons;
		const pactOfTheBlade = actor.items.filter(i => i.name === "Pact of the Blade");
		if (pactOfTheBlade)
			...
			buttons:
			{
				Ok:
				{
					...
					callback: async (html) => {
						...
						ChatMessage.create({
							content: `${token.name}'s ${selectedItem.name} is imbued with your patron's power`,
