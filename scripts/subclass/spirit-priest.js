/*
	The target regains Hit Points equal to two rolls of your Bardic Inspiration die, and one of the following conditions
	of your choice ends on the target: Blinded, Charmed, Deafened, Paralyzed, Poisoned, or Stunned.
*/
const version = "14.5.0";
const optionName = "Priest Spirit";
const condition_list = new Set(["blinded", "charmed", "deafened", "paralyzed", "poisoned", "stunned"]);
    ...
    if (args[0].macroPass === "postActiveEffects") {
        for (let targetToken of workflow.targets) {
            ...
            if (removedCondition) {
                ...
                let chatContent = `<div class="midi-qol-nobox"><div class="midi-qol-flex-container"><div>Removed ${removedCondition} from: </div><div class="midi-qol-target-npc midi-qol-target-name" id="${targetToken.id}"> ${targetToken.actor.name}</div><div><img src="${targetToken.document.texture.src}" width="30" height="30" style="border:0px" /></div></div></div>`;
                ChatMessage.create({
                    content: chatContent,
