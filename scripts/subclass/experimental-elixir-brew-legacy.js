...
try {
    if (args[0].macroPass === "postActiveEffects") {
        ...
        let tempItem = elixirItem.toObject();
        tempItem.system.quantity = 2;
        ...
        // handle Restorative Reagents
        if (actor.items.getName("Restorative Reagents")) {
            const restorativeActivity = results[0].system.activities.getName("Restorative Reagents");
            await restorativeActivity.update({
                "healing.bonus" : intMod
            });

            await healingActivity.update({
                "midiProperties.triggeredActivityId" : 'restorative-reagents',
                "midiProperties.triggeredActivityTargets" : 'targets',
                "midiProperties.triggeredActivityConsume" : false,
                "midiProperties.triggeredActivityConfigure" : false
            });

            await swiftnessActivity.update({
                "midiProperties.triggeredActivityId" : 'restorative-reagents',
                "midiProperties.triggeredActivityTargets" : 'targets',
                "midiProperties.triggeredActivityConsume" : false,
                "midiProperties.triggeredActivityConfigure" : false
