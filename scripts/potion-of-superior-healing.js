const version = "0.1.0";
const optionName = "Potion of Superior Healing";
			...
			new Dialog({
				...
				buttons: {
					one: {
						...
						callback: async (html) => {
							let healDamage = new Roll(`8d4+8`).evaluate({ async: false });
					...
					two: {
						...
						callback: async (html) => {
							let healDamage = new Roll(`40`).evaluate({ async: false });
							await new MidiQOL.DamageOnlyWorkflow(tactor, token, healDamage.total, healingType, [target], healDamage, { flavor: `(${CONFIG.DND5E.healingTypes[healingType]})`, itemCardId: args[0].itemCardId, useOther: false });
						}
					},
					three: {
						...
						callback: async (html) => {
							...
							let healDamage = new Roll(`40+${conMod}`).evaluate({ async: false });
							await new MidiQOL.DamageOnlyWorkflow(tactor, token, healDamage.total, healingType, [target], healDamage, { flavor: `(${CONFIG.DND5E.healingTypes[healingType]})`, itemCardId: args[0].itemCardId, useOther: false });
						}
					},
					cancel: {
