...
try {
    if (args[0].macroPass === "preItemRoll") {
        ...
        let targetChoices = [];
        const allies = MidiQOL.findNearby(1, token, 30);
        ...
        let choices = await warpgate.menu(menuOptions, {
            title: `${optionName} - max of ${maxTargets}`,
            options: {height: "100%"}
        });
        ...
        for (var i = 0; i < huntChoices.length && i < maxTargets; i++) {
            ...
            if (tactor) {
                await applyDamageBonus(tactor, actor.uuid);
...
async function applyDamageBonus(targetActor, actorId) {
    const effectData = {
        label: optionName,
