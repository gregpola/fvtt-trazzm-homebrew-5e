...
try {
	...
	if (lastArg.targets.length > 0) {
		...
		const aoeTargets = await canvas.tokens.placeables.filter((placeable) =>	canvas.grid.measureDistance(target, placeable) <= 9.5 
	  ...
	  ui.notifications.error(`${optionName}: No target selected: unable to automate burst effect.`);
	}
