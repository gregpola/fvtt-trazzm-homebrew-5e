...
try {
	if (args[0].macroPass === "postActiveEffects") {
		...
		let dialog = new Promise((resolve, reject) => {
			new Dialog({
				title: `${optionName}`,
				content: "<p>What type of area is targeted?</p>",
				buttons:
				{
					air: {
						label: '<p>Air</p>',
					...
					earth: {
						label: '<p>Earth</p>',
						callback: () => { resolve("earth") }
					},
					fire: {
						label: '<p>Fire</p>',
						callback: () => { resolve("fire") }
					},
					water: {
						label: '<p>Water</p>',
		...
		if (choice) {
			...
			switch (choice) {
				case 'air':
					if (spellLevel < 6) {
						if (rollResult < 5)
							summonedId = "Compendium.fvtt-trazzm-homebrew-5e.homebrew-creatures.v4rQ1CQE42yEl1bj";
						else
							summonedId = "Compendium.fvtt-trazzm-homebrew-5e.homebrew-creatures.LXHOQme8MMELmEUF";
					}
					else if (spellLevel === 6)
						summonedId = "Compendium.fvtt-trazzm-homebrew-5e.homebrew-creatures.sbZYf8JBiUAAegrM";
					else
						summonedId = "Compendium.fvtt-trazzm-homebrew-5e.homebrew-creatures.jrTQpI7SWn5qnAog";
					break;
				...
				case 'fire':
					if (spellLevel < 7) {
						if (rollResult < 5)
							summonedId = "Compendium.fvtt-trazzm-homebrew-5e.homebrew-creatures.uCSmT6sljSrBXLwt";
						else
							summonedId = "Compendium.fvtt-trazzm-homebrew-5e.homebrew-creatures.rFt46dslkVN8DJGk";
					}
					else
						summonedId = "Compendium.fvtt-trazzm-homebrew-5e.homebrew-creatures.ts1PY3MzJEb3nNu8";
					break;

				case 'water':
					if (spellLevel < 7)
						summonedId = "Compendium.fvtt-trazzm-homebrew-5e.homebrew-creatures.TVjN6jTT3wUm1TKk";
					else
						summonedId = "Compendium.fvtt-trazzm-homebrew-5e.homebrew-creatures.AykY0KoJldXnJq2R";
					break;
			}
