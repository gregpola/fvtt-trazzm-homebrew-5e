...
try {
    if (args[0].tag === 'OnUse') {
        ...
        if (["mwak", "msak"].includes(scope.rolledItem.system.actionType)) {
            const attacker = scope.rolledItem.actor;
            ...
            const roll = await new Roll(`2d8[${dmgType}]`).toMessage({
                flavor: DIV.innerHTML,
                speaker: ChatMessage.getSpeaker({scene: game.canvas?.scene, actor: defenderActor, token: defenderToken})
            });
