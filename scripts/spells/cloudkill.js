...
try {
    if (args[0] === "each" && lastArgValue.turn === 'startTurn') {
        // get the summon
        const theCloudkill = actor.summonedCreatures.find(s => s.name === 'Cloudkill')
        if (theCloudkill) {
            const theToken = MidiQOL.tokenForActor(theCloudkill);
            if (theToken) {
                await HomebrewMacros.pushTarget(token, theToken, 2);
            }
        }
    }
