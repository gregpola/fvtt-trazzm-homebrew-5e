			...
			new Dialog({
				...
				buttons: {
					one: {
						...
						callback: async (html) => {
							let healDamage = new Roll(`10d4+20`).evaluate({ async: false });
					...
					two: {
						...
						callback: async (html) => {
							let healDamage = new Roll(`60`).evaluate({ async: false });
							await new MidiQOL.DamageOnlyWorkflow(tactor, token, healDamage.total, healingType, [target], healDamage, { flavor: `(${CONFIG.DND5E.healingTypes[healingType]})`, itemCardId: args[0].itemCardId, useOther: false });
						}
					},
					three: {
						...
						callback: async (html) => {
							...
							let healDamage = new Roll(`60+${conMod}`).evaluate({ async: false });
							await new MidiQOL.DamageOnlyWorkflow(tactor, token, healDamage.total, healingType, [target], healDamage, { flavor: `(${CONFIG.DND5E.healingTypes[healingType]})`, itemCardId: args[0].itemCardId, useOther: false });
						}
					},
					cancel: {
